'''''
BLINKY : Greedy Best first search
INKY : Hill climbing algorithm (made dumb) 
'''
...
class Ghost:
    ...
    def move_inky(self):
        ...
        if random.random() < 0.6:  
            best_move = random.choice(adjacent_positions)
        else:
            best_move = None
            min_distance = self.euclidean(current_pos, pacman_pos)
            #loop thro neighbors
            for neighbor in adjacent_positions:
                distance = self.euclidean(neighbor, pacman_pos)

                if best_move is None or random.random() < 0.6:  
                    best_move = neighbor
        ...
        if best_move:
            self.x_pos += (best_move[0] - current_pos[0]) * 0.5
            self.y_pos += (best_move[1] - current_pos[1]) * 0.5
...
def draw_misc():
    ...
    if (time.time()-st1<=10):
        time_text = font.render(str(int(10-(time.time()-st1)))+' Seconds until inky arrives' ,True,'lightblue')
...
while run:
    ...
    if (abs((player_y//15)-blinky_xc)==1 and abs((player_x//20)-blinky_yc)==1):
        ...
        en=time.time()
        break
    if (time.time()-st1>=10 and abs((player_y//15)-inky_xc)==1 and abs((player_x//20)-inky_yc)==1):
